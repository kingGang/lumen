...
impl AppState {
    ...
    fn toggle_maximize_pane(&mut self, pi: usize) {
        // 仅「进入最大化」分支会强制改焦点（还原 / 单格 no-op 不改焦点，
        // 不打断上报拖动）。先在不可变借用里判定，再在改 focused 前补发。
        let will_change_focus = {
            let tab = &self.tabs[self.active_tab];
            if pi >= tab.panes.len() {
                return; // 防御：结构刚变更的过渡帧
            }
            tab.maximized.is_none() && tab.panes.len() > 1
        };
        if will_change_focus {
            // 改焦点前补发鼠标上报拖动的 Release（避免旧焦点窗格幻影按住）。
            self.release_held_report_buttons();
        }
        let tab = &mut self.tabs[self.active_tab];
        if pi >= tab.panes.len() {
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                // 鼠标上报（Button 拖动 / Any 任意移动）开启时，移动交给程序，
                // 不走本地拖选 / 链接 hover。但一旦本地拖选 / footer 拖选已经
                // 起手（按下时归了本地），本次拖动全程交给本地分支收尾，绝不
                // 中途被上报劫持（否则跨窗格 / 中途松 Shift 会冻结、丢弃选区）。
                #[allow(unused_mut)]
            ...
            WindowEvent::MouseInput {
                ...
                (MouseButton::Left, ElementState::Pressed) => {
                    ...
                    // 鼠标上报开启（且非 Shift）：本次按下交给程序处理，
                    // 不建本地选区（已在上面完成聚焦该窗格）。
                    if state.report_mouse_button(MouseButton::Left, true) {
                        return;
                    }
                ...
                (MouseButton::Left, ElementState::Released) => {
                    ...
                    // 鼠标上报开启（且非 Shift）：把释放编码发给程序。
                    if state.report_mouse_button(MouseButton::Left, false) {
                        return;
                    }
                ...
                (MouseButton::Right, ElementState::Pressed) => {
                    ...
                    // 鼠标上报开启（且非 Shift）：右键按下交给程序，不走
                    // 本地复制/粘贴（程序可能用右键弹自己的菜单）。
                    if state.report_mouse_button(MouseButton::Right, true) {
                        return;
                    }
                ...
                (MouseButton::Middle, ElementState::Pressed) => {
                    // 与左/右键一致先做焦点仲裁（F5）：点中哪个窗格就聚焦哪个，
                    // 否则中键上报会写给非焦点窗格、且释放回退焦点窗格时与按下
                    // 目标对不齐（留下幻影按住）。点在 egui 面板上则不聚焦。
